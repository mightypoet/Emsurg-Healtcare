...
export function formatDriveImageUrl(url: string): string {
  ...
  // If already a formatted thumbnail CDN link with sizing parameter
  if (trimmed.includes('lh3.googleusercontent.com/d/') && trimmed.includes('=')) {
    return trimmed;
  }

  // Match standard /file/d/{id}, /d/{id}, id={id}, or /open?id={id}
  const match =
