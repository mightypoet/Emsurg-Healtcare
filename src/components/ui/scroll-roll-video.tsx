...
export function ScrollRollVideo({
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6 bg-slate-900/70 backdrop-blur-md border border-slate-800 rounded-2xl p-2 sm:p-2.5">
      ...
      <motion.div
        ...
        className="relative aspect-[16/10] sm:aspect-[21/10] w-full overflow-hidden bg-[#030712] border border-white/20 shadow-2xl shadow-sky-950/40 select-none group transform-gpu will-change-transform"
      >
        ...
        <button
          type="button"
          onClick={toggleSound}
          className="absolute top-4 right-4 z-30 p-2.5 rounded-full bg-slate-900/70 hover:bg-slate-900 text-white/90 backdrop-blur-md border border-white/15 transition-all shadow-md hover:scale-105"
        ...
        <div className="absolute top-4 left-4 z-20 flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-950/70 backdrop-blur-md border border-white/10 text-white text-[11px] font-semibold tracking-wider uppercase">
          <span className="w-1.5 h-1.5 rounded-full bg-sky-400 animate-ping" />
          <span className="text-sky-300">Cleanroom Operations</span>
        </div>
            ...
            >
              <button
                ...
                className={`group relative flex items-center justify-center w-8 h-8 sm:w-9 sm:h-9 rounded-full transition-all duration-300 focus:outline-none ${
                  isActive
                    ? "bg-sky-400 text-slate-950 scale-110 shadow-[0_0_20px_rgba(56,189,248,0.8)]"
                    : "bg-slate-900/80 text-sky-300 border border-sky-400/40 hover:bg-sky-500 hover:text-white hover:scale-105"
                }`}
              ...
              >
                ...
                <span
                  className={`absolute inset-0 rounded-full animate-ping pointer-events-none opacity-60 ${
                    isActive ? "bg-sky-400" : "bg-sky-500/40"
                  }`}
                />
              ...
              <div
                className={`hidden md:block absolute top-full left-1/2 -translate-x-1/2 mt-2 px-2.5 py-1 rounded-md text-[10px] font-bold tracking-wider uppercase whitespace-nowrap backdrop-blur-md border transition-all pointer-events-none ${
                  isActive
                    ? "bg-sky-950/90 text-sky-200 border-sky-500/50 shadow-md"
                    : "bg-slate-950/70 text-slate-300 border-white/10 opacity-70 group-hover:opacity-100"
                }`}
              >
                {chap.title}
              </div>
        ...
        <AnimatePresence mode="wait">
          {showDetailCard && activeChapter && (
            <motion.div
              ...
              className="absolute bottom-4 left-4 right-4 sm:right-auto sm:max-w-md z-30 bg-slate-950/85 backdrop-blur-xl border border-sky-500/30 rounded-2xl p-4 sm:p-5 text-white shadow-2xl shadow-slate-950/80"
            >
              ...
              <p className="text-xs text-slate-300 leading-relaxed line-clamp-2 sm:line-clamp-3 mb-3">
              ...
              <div className="space-y-1 mb-4 hidden sm:block">
                {activeChapter.specs.map((spec, i) => (
                  <div key={i} className="flex items-center gap-1.5 text-[11px] text-sky-200">
