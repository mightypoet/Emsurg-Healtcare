...
export default function SwissExpertiseSpotlight() {
  const [activeHotspotId, setActiveHotspotId] = useState<string | null>("bonesurg");
  ...
  const activeSpot = hotspots.find((h) => h.id === activeHotspotId) || hotspots[0];

  return (
    <section id="our-expertise" className="bg-white py-16 sm:py-24 md:py-28 border-t border-slate-100 select-none overflow-hidden">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        {/* Mobile Filter Pills for Easy Touch Ergonomics */}
        <div className="flex md:hidden items-center gap-2 overflow-x-auto no-scrollbar pb-3 mb-6 px-1">
          {hotspots.map((spot) => (
            <button
              key={spot.id}
              type="button"
        ...
        <div className="relative w-full max-w-5xl mx-auto mb-8 sm:mb-12">
          {/* Ground cast shadow */}
          <div 
            ...
            {hotspots.map((spot) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Popover Card (Desktop / Tablets) */}
                  {isActive && (
                    <div
                      onMouseLeave={() => setActiveHotspotId(null)}
                      className="hidden md:block absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-72 sm:w-80 p-4 bg-white/95 backdrop-blur-md rounded-2xl border border-slate-200 shadow-xl text-left z-30 animate-in fade-in zoom-in-95 duration-200"
                    >
        ...
        {/* Dedicated Mobile Specification Card (Zero Overflow, Touch Optimized) */}
        {activeSpot && (
          <div className="md:hidden mb-8 p-5 bg-slate-50 rounded-2xl border border-slate-200 shadow-sm text-left">
            <div className="flex items-center justify-between mb-2">
              <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-blue-700 bg-blue-100/70 px-2.5 py-0.5 rounded-full border border-blue-200">
                <Sparkles className="w-2.5 h-2.5 text-blue-600" />
                {activeSpot.badge}
              </span>
              <span className="text-[10px] font-semibold text-slate-400">Clinical Focus</span>
            </div>
            ...
            <div className="pt-3 border-t border-slate-200/80 flex items-center justify-between">
              <span className="text-xs font-semibold text-emerald-700 flex items-center gap-1">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /> Clinical Standard
              </span>
              <Link
                to={activeSpot.link}
                className="text-xs font-bold text-blue-600 hover:text-blue-700 inline-flex items-center gap-1 py-1"
              >
